...
/* ===========================
   AT24 – 3D Carousel Helper (Home pages)
   ใช้กับ: โปรโมชั่น, บริการ, สินค้าแนะนำ, รีวิว, FAQ
   วางท้ายไฟล์ public.js ได้เลย
   =========================== */
(() => {
  if (window.at24MakeCarousel3D) return; // กันซ้ำ

  function makeCarousel3D(listEl){
    if (!listEl || listEl.dataset.at24Inited) return null;

    // หาไอเท็มที่เป็นการ์ด/รายการ
    const items = [...listEl.querySelectorAll('.card, .product-card, .review-card, .accordion-item')]
      .filter(el => el.offsetParent !== null);
    if (items.length < 4) return null; // น้อยไปไม่ทำ 3D
    ...
    const radius = parseFloat(rootStyle.getPropertyValue('--at24-3d-radius')) || 360;
    slides.forEach((s, i) => {
      s.style.transform = `rotateY(${i*step}deg) translateZ(${radius}px)`;
    });

    // สถานะหมุน/สไลด์ปัจจุบัน
    let curr = 0, rot = 0;
  ...
  window.at24MakeCarousel3D = makeCarousel3D;

  // ===== Auto-attach กับแต่ละส่วนของหน้า Home =====
  function tryAttach(){
    // 1) โปรโมชั่น
    const promo = document.querySelector('#promotions, .promotions, section.promotions');
    if (promo) at24MakeCarousel3D(promo.querySelector('.row, .list, .cards, .grid') || promo);

    // 2) บริการ (บนหน้าเว็บ)
    const homeSrv = document.querySelector('#home-services, .home-services, section.services, .services');
    if (homeSrv) at24MakeCarousel3D(homeSrv.querySelector('.row, .list, .cards, .grid') || homeSrv);

    // 3) สินค้าแนะนำ
    const featured = document.querySelector('#featured-products, .featured-products, #featured, .featured');
    if (featured) at24MakeCarousel3D(featured.querySelector('.row, .list, .cards, .grid') || featured);

    // 4) รีวิว
    const reviews = document.querySelector('#reviews, .reviews, section.reviews');
    if (reviews) at24MakeCarousel3D(reviews.querySelector('.row, .list, .cards, .grid') || reviews);

    // 5) FAQ (ถ้าเป็น accordion ก็จับ container ตรง ๆ ได้)
    const faqs = document.querySelector('#faq, #faqs, .faqs, section.faq');
    if (faqs) at24MakeCarousel3D(faqs.querySelector('.accordion, .row, .list, .cards') || faqs);
  }

  // รันหลัง DOM พร้อม และเฝ้าดู content ที่โหลดจาก Firestore
