...
function productCardHtml(p) {
  const img = p.image || (p.gallery?.[0]) || '/assets/placeholder.jpg';
  const priceOld = Number(p.price || 0);
  const priceNow = Number(p.sale ?? priceOld);
  const showOld  = priceOld > priceNow;
  const percent  = showOld && priceOld > 0 ? Math.round((1 - priceNow / priceOld) * 100) : 0;
  const tags = (p.tags || []).map(t => `<span class="badge rounded-pill text-bg-light me-1">#${t}</span>`).join('');
  const stockLine = (typeof p.stock === 'number') ? `<div class="small text-muted">สต็อก: ${p.stock}</div>` : '';
  const promoStart = p.promoStart?.toDate ? p.promoStart.toDate()
                   : (p.promoStart ? new Date(p.promoStart) : null);
  const promoLine = promoStart ? `<div class="small text-muted">เริ่มโปรโมชัน: ${promoStart.toLocaleString('th-TH')}</div>` : '';
  ...
  <div class="col-12 col-md-6 col-lg-4">
    <div class="card product-card h-100 shadow-sm">
      <div class="position-relative ratio ratio-16x9 overflow-hidden">
        ...
        ${percent ? `<span class="badge bg-danger position-absolute top-0 start-0 m-2">ลด ${percent}%</span>` : ''}
      </div>
      <div class="card-body">
        <h6 class="mb-1">${escapeHtml(p.name || '-')}</h6>
        <div class="d-flex align-items-baseline gap-2 mb-1">
          ${showOld ? `<span class="text-muted text-decoration-line-through small">฿${priceOld.toLocaleString()}</span>` : ''}
          <span class="fw-bold text-primary">฿${priceNow.toLocaleString()}</span>
        </div>
        ${stockLine}
        ${promoLine}
        <div class="mb-2">${tags}</div>
        <button class="btn btn-primary w-100" data-id="${p.id}" data-action="openProduct">ดูรายละเอียด</button>
      </div>
    </div>
  </div>`;
}

// ถ้าในไฟล์คุณยังไม่มี
...
async function loadHomeProducts() {
  ...
  const qRef = query(
    collection(db,'products'),
    where('isActive','==',true),
    where('featured','==',true), // ติ๊ก “แนะนำ” ถึงจะโชว์
    orderBy('rank','asc'),
    limit(3)
  );
  const snap = await getDocs(qRef);
  wrap.innerHTML = snap.empty
    ? `<div class="text-muted">ไม่มีสินค้าแนะนำ</div>`
    : snap.docs.map(d => productCardHtml({ id:d.id, ...d.data() })).join('');
}

document.addEventListener('click', async (ev) => {
  const btn = ev.target.closest('[data-action="openProduct"]');
  if (!btn) return;
  const ref  = doc(db,'products', btn.dataset.id);
  const snap = await getDoc(ref);
  if (!snap.exists()) return;
  // ใช้ตัวเดียวกับ shop
  showProductModal({ id:snap.id, ...snap.data() });
});

document.addEventListener('DOMContentLoaded', () => {
  loadHomeProducts(); // เรียกใช้ในหน้า index
});
