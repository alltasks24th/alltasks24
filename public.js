...
function bindRealtime(){
  ...
  onSnapshot(collection(db,'services'), snap=>{
    ...
    wrap.innerHTML=''; if(mods) mods.innerHTML='';

    snap.forEach(s=>{
      const d = s.data() || {}; const id = s.id;
      const name = d.name || ''; const category = d.category || '';
      ...
      if(mods){
        ...
        mods.insertAdjacentHTML('beforeend', `<div class="modal fade" id="svc-${id}" tabindex="-1" aria-labelledby="svc-label-${id}" aria-hidden="true">
          <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
              <div class="modal-header"><h5 id="svc-label-${id}" class="modal-title">${name}</h5>
                <button class="btn-close" data-bs-dismiss="modal" aria-label="ปิด"></button></div>
