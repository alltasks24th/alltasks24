...
import { el, els, fmtDate, id, pill } from './utils.js';
...
function bindServices(){
  ...
  el('#btnAddService').onclick = async ()=>{
    ...
    const description = prompt('รายละเอียดสั้น');
    const imageUrl = prompt('ลิงก์รูปภาพ (ใส่ลิงก์/GitHub)');
...
function bindUsers(){
  ...
  el('#btnAddUser').onclick = async ()=>{
    ...
    const email = prompt('อีเมล (ไม่บังคับ)');
    const role = prompt('บทบาท (viewer/admin/owner)','viewer')||'viewer';
...
function bindPromos(){
  ...
  el('#btnAddPromo').onclick = async ()=>{
    ...
    const imageUrl = prompt('ลิงก์รูปภาพ');
...
function bindFaq(){
  ...
  el('#btnAddFaq').onclick = async ()=>{
    ...
    await addDoc(ref, { q, a, createdAt: serverTimestamp() });
    reload();
  };
  reload();
}
...
function bindSettings(){
  ...
  getDoc(ref).then(snap=>{
    ...
    el('#settingsForm').siteName.value = d.siteName||'';
    el('#settingsForm').heroText.value = d.heroText||'';
    el('#settingsForm').phone.value = d.phone||'';
    el('#settingsForm').line.value = d.line||'';
    el('#settingsForm').facebook.value = d.facebook||'';
    el('#settingsForm').mapUrl.value = d.mapUrl||'';
    el('#settingsForm').mediaPolicy.value = d.mediaPolicy||'';
  });
...
function bindChats(){
  ...
  onSnapshot(query(collection(db,'chatThreads'), orderBy('createdAt','desc')), snap=>{
    ...
    snap.forEach(d=>{
      ...
      item.innerHTML = `<span>Thread: ${d.id.slice(0,6)}…</span><span class="small text-muted">${t.status||''}</span>`;
      item.href='#'; item.onclick=()=>openThread(d.id);
...
function openThread(id){
  activeThreadId = id;
  el('#chatRoom').style.display='block';
  el('#chatRoomTitle').textContent = 'ห้องแชท #' + id.slice(0,6);
  const body = el('#chatRoomBody');
  onSnapshot(query(collection(db,'chatThreads', id, 'messages'), orderBy('createdAt')), snap=>{
    body.innerHTML='';
    snap.forEach(m=>{
      const d=m.data();
      const who = d.sender==='user'? 'primary-subtle' : (d.sender==='agent'?'success-subtle':'secondary-subtle');
      body.insertAdjacentHTML('beforeend', `<div class="p-2 rounded mb-1 bg-${who}">${d.sender}: ${d.text}</div>`);
      body.scrollTop = body.scrollHeight;
    });
  });
  el('#chatReplySend').onclick = async ()=>{
    const text = el('#chatReply').value.trim(); if(!text) return;
...
function bindRowActions(container, ref){
  el(container).onclick = async (e)=>{
    const btn = e.target.closest('button'); if(!btn) return;
    const id = btn.dataset.id; const act = btn.dataset.act||'';
    const dref = doc(ref, id);
    if(act==='del'){
      if(confirm('ลบรายการนี้?')) await deleteDoc(dref);
      e.target.closest('tr')?.remove();
      return;
    }
    if(act.startsWith('set:')){
      const [,val] = act.split(':');
      if(ref.id==='bookings') await updateDoc(dref, { status: val });
      if(ref.id==='tickets') await updateDoc(dref, { status: val });
      if(ref.id==='reviews' && val==='approve') await updateDoc(dref, { approved: true });
      return;
    }
    if(act.startsWith('role:')){
      const [,role] = act.split(':');
      await updateDoc(dref, { role });
      return;
    }
    // edit generic (prompt-based)
    if(act==='edit'){
      const snap = await getDoc(dref); const data = snap.data()||{};
      for(const k of Object.keys(data)){
        if(['createdAt','start','end'].includes(k)) continue;
        const nv = prompt(`แก้ไข ${k}`, data[k]??'');
        if(nv===null) continue; data[k]=nv;
      }
